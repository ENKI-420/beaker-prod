...
export class RedisConnectionPool {
  ...
  private async createConnection(): Promise<PooledConnection> {
    try {
      const url = this.config.url || process.env.KV_REST_API_URL || process.env.KV_URL || process.env.REDIS_URL || ""
   ...
   */
  public releaseConnection(client: Redis): void {
  ...
  public async close(): Promise<void> {
    ...
    for (const conn of this.connections) {
      try {
        // Upstash Redis doesn't have a close method, but we'll keep this for future compatibility
        if (typeof conn.client.disconnect === "function") {
